...
import { ArrowUp, Car, CreditCard, Gift, Home, PiggyBank, ShoppingCart, Utensils, Zap, IndianRupee, ArrowDown, TrendingUp, Clock, RefreshCw, Info, ChevronUp, ChevronDown, PieChart } from "lucide-react";
...
const Layout = ({ onLogout, user}) => {
  ...
  //to fetch transaction from the server side
...
//to add transaction either income or expense
  ...
  //get stats data according to the time
  const stats = useMemo(() => {
    const now = new Date();
    ...
    <div className={styles.layout.mainContainer(sidebarCollapsed)}>
      ...
      <div className={styles.statCards.grid}>
        ...
        <div className={styles.statCards.card}>
          <div className={styles.statCards.cardHeader}>
            <div>
              ...
              <p className={styles.statCards.cardValue}>
                ₹{(stats?.last30DaysIncome || 0).toLocaleString("en-IN", { maximumFractionDigits: 2 })}
              </p>
            </div>
        ...
        <div className={styles.statCards.card}>
          ...
          <p className={styles.statCards.cardFooter}>
            <span className={`${styles.colors.expenseChange(stats.expenseChange)} front-medium`}>
